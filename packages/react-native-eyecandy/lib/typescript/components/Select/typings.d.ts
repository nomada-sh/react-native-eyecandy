...
import type { PickerSelectProps as SelectBaseProps, Item as ItemBase } from 'react-native-picker-select';
...
export declare type SelectIcon = FC<{
    size: number;
    stroke?: string;
}>;
export interface SelectProps<ValueType = unknown> extends Omit<SelectBaseProps, 'items' | 'onValueChange' | 'style' | 'placeholder' | 'value'> {
